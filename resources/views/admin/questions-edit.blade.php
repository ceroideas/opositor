...
@section('body')
	...
	<div class="row no-margin">
		<div class="col-12 header-1">
			<a href="{{url()->previous()}}" class="goback-link"><i class="fa fa-angle-left"></i></a>
			<!-- <a href="{{url('/admin/temas/' . $tema->id . '/subtema/' . $subtema->id)}}" class="goback-link"><i class="fa fa-angle-left"></i></a> -->
			<h2 style="text-align:center;"><strong>Editar preguntas</strong></h2>
			<span></span>
		</div>
	</div>
		...
		<div class="col-12">
			...
			<section class="panel max-1000px ma">
				<header class="panel-heading">
					<strong> Editar la pregunta en {{$subtema->title}} </strong>
<!--
...
-->
				...
				<form method="POST">
					...
					@switch($subtema->type)
						...
						@case('true_or_false')
							@include('inc._true_or_false_edit_question_template')
							@break

						@case('flashcards')
							@include('inc._flashcards_edit_question_template');
							@break

						@case('multiple_choice')
							@include('inc._multiple_choice_edit_question_template');
							@break
					@endswitch
